...
namespace ExpandableCalculator
{
    public static class Calculator
    {
        ...
        {
            var fractionToPercent = Regex.Match(expr, @"([\d\.]+)\s*/\s*([\d\.]+)\s*(to|as)\s*percent",
            RegexOptions.IgnoreCase
        );

            if (fractionToPercent.Success)
            {
                double num = double.Parse(fractionToPercent.Groups[1].Value, CultureInfo.InvariantCulture);
                double den = double.Parse(fractionToPercent.Groups[2].Value, CultureInfo.InvariantCulture);

                double percent = Math.Round((num / den) * 100, 2);
        ...
        private static double ParseNumber(string token)
        {
            if (token.EndsWith("%"))
            {
                double percent = double.Parse(token.TrimEnd('%'), CultureInfo.InvariantCulture);
                return percent / 100.0; // percent → decimal
            }
        ...
        private static List<string> ToRPN(string expr)
        {
            ...
            while (i < expr.Length)
            {
                ...
                if (char.IsDigit(expr[i]))
                {
                    int start = i;
                    while (i < expr.Length &&
                          (char.IsDigit(expr[i]) || expr[i] == '.' || expr[i] == '/' || expr[i] == '%'))
                    {
                        i++;
                    }
